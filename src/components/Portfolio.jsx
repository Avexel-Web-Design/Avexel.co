...
const ProjectCard = ({ title, description, image, tags, websiteUrl, caseStudyUrl, orientation = 'right', teamName = '' }) => {
  ...
  return (
    <div 
      ...
      className={`group relative reveal transition-all duration-1000 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
    ...
    >
      ...
      <div className="relative bg-dark/70 backdrop-blur-md rounded-xl border border-white/10 overflow-hidden shadow-2xl group-hover:border-white/20 transition-all duration-500">
        <div className="flex flex-col md:flex-row md:items-stretch gap-6 md:gap-10 p-6 sm:p-8 lg:p-10">
          ...
          <div 
            className={`relative w-full md:w-5/12 ${orientation === 'left' ? 'md:order-1' : 'md:order-2'}`}
          >
            <div className="relative aspect-video md:aspect-square overflow-hidden rounded-xl shadow-xl">
              ...
              <div className={`absolute inset-0 bg-gradient-to-t from-dark/90 via-dark/50 to-transparent ${isHovered ? 'opacity-0' : 'opacity-70'} transition-opacity duration-700 z-10`}></div>
              ...
              <div className="absolute inset-0 overflow-hidden flex items-center justify-center p-4">
                <img 
                  src={image} 
                  alt={title}
                  className={`max-w-full max-h-full w-auto h-auto object-contain transition-all duration-1000 ease-out ${isHovered ? 'scale-110 filter saturate-100' : 'scale-100 filter saturate-75'}`}
                />
              </div>
              
              {/* Enhanced corner accents with gradients */}
              <div className={`absolute top-0 left-0 w-12 h-12 border-t-2 border-l-2 border-primary-400/70 rounded-tl opacity-0 group-hover:opacity-100 transition-all duration-700 ${isHovered ? 'scale-100' : 'scale-50'}`}>
                <div className="absolute inset-0 bg-gradient-to-br from-primary-400/20 to-transparent rounded-tl"></div>
              </div>
              <div className={`absolute bottom-0 right-0 w-12 h-12 border-b-2 border-r-2 border-secondary-400/70 rounded-br opacity-0 group-hover:opacity-100 transition-all duration-700 ${isHovered ? 'scale-100' : 'scale-50'}`}>
          ...
          <div className={`flex flex-col justify-center w-full md:w-7/12 ${orientation === 'left' ? 'md:order-2' : 'md:order-1'}`}>
            <div className="mb-4">
...
const Portfolio = () => {
  ...
  return (
    <section id="work" className="py-24 bg-black relative overflow-hidden scroll-mt-16">
        ...
        <div className="grid gap-12 reveal">
          ...
          <div className="relative glass-morphism p-10 rounded-xl border border-white/5 text-center reveal">
            ...
            <p className="text-gray-300 mb-6 max-w-2xl mx-auto">
              We're constantly expanding our portfolio with new projects. Contact us to discuss how we can bring your vision to life.
            </p>
          </div>
        </div>
